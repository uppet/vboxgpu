...
def write_method(reg, param):
    """Return the stream write call for a single parameter."""
    if reg.is_handle(param.type_name):
        return f'w->writeU64(static_cast<uint64_t>({param.name}));'
    sz = reg.get_scalar_size(param.type_name)
    if sz == 4:
        return f'w->writeU32(static_cast<uint32_t>({param.name}));'
    elif sz == 8:
        return f'w->writeU64(static_cast<uint64_t>({param.name}));'
    return None  # complex type, needs special handling

def read_method(reg, param):
    """Return the stream read expression for a single parameter."""
    if reg.is_handle(param.type_name):
        return 'r->readU64()'
    sz = reg.get_scalar_size(param.type_name)
    if sz == 4:
...
def cast_read(reg, param, read_expr):
    """Wrap read expression with appropriate cast."""
    if reg.is_handle(param.type_name):
        return read_expr  # stays as uint64_t
    if param.type_name == 'float':
        return f'r->readF32()'
    if param.type_name in ('int32_t',):
        return f'static_cast<int32_t>({read_expr})'
    sz = reg.get_scalar_size(param.type_name)
    if sz == 4 and param.type_name not in ('uint32_t',):
        return f'static_cast<{param.type_name}>({read_expr})'
    if sz == 8 and param.type_name not in ('uint64_t',):
        return f'static_cast<{param.type_name}>({read_expr})'
    return read_expr

def is_simple_command(reg, cmd):
    """Check if all params are simple scalars or handles (no structs/arrays/pointers)."""
    for p in cmd.params:
        if p.is_pointer:
            return False
        if reg.is_struct(p.type_name):
            return False
        if reg.is_handle(p.type_name):
            continue
        if reg.get_scalar_size(p.type_name) == 0:
            return False
    return True

def gen_simple_encoder(reg, cmd):
    """Generate encoder for a simple command (all params are scalars/handles)."""
    lines = []
    lines.append(f'static inline void vn_encode_{cmd.name}(VnStreamWriter* w')
    for p in cmd.params:
        if reg.is_handle(p.type_name):
            lines.append(f'    , uint64_t {p.name}')
        elif p.type_name == 'float':
            lines.append(f'    , float {p.name}')
        elif reg.get_scalar_size(p.type_name) == 8:
            lines.append(f'    , uint64_t {p.name}')
        else:
            lines.append(f'    , uint32_t {p.name}')
    lines.append(')')
    lines.append('{')
    for p in cmd.params:
        if reg.is_handle(p.type_name):
...
def gen_simple_decoder(reg, cmd):
    """Generate decoder struct + decode function for a simple command."""
    lines = []
    # Struct to hold decoded params
...
def gen_encode_header(reg, apis):
    ...
    for name in sorted(apis.keys()):
        ...
        if is_simple_command(reg, cmd):
            lines.append(gen_simple_encoder(reg, cmd))
            lines.append('')
            generated.append(name)
        else:
            skipped.append(f'// TODO: {name} — complex params, needs manual or extended codegen')
...
def gen_decode_header(reg, apis, generated_names):
    ...
    for name in sorted(generated_names):
        cmd = reg.commands.get(name)
        if cmd and is_simple_command(reg, cmd):
            lines.append(gen_simple_decoder(reg, cmd))
            lines.append('')
