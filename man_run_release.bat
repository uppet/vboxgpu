@echo off
REM ============================================================
REM  VBox GPU Bridge - Manual Run Script (RELEASE build)
REM  Usage: man_run_release.bat [test_name]
...
set HOST_EXE=%ROOT%\build\host\Release\vbox_host_server.exe
set ICD_DLL=%ROOT%\build\guest_vk_icd\Release\vbox_vulkan.dll
set TEST_ARGS=
...
set ICD_DLL=%ROOT%\build32\guest_vk_icd\Release\vbox_vulkan.dll
goto :selected
...
echo [2/3] Starting host server (Release)...
